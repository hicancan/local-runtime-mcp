...
type Point = { x: number; y: number; source: DebugSource };
...
async function snapshot(tabId: number, maxElements: unknown, maxText: unknown) {
  ...
  for (const source of sources) {
    ...
    for (const node of tree.nodes || []) {
      ...
      if (value && value !== name) texts.push(value);
      if (!node.backendDOMNodeId || !interactiveAXRoles.has(role)) continue;
      if (elements.length >= limit) {
        elementsTruncated = true;
        continue;
      }
      const ref = `${snapshotId}:e${elements.length + 1}`;
      const properties = Object.fromEntries((node.properties || []).map((item: any) => [item.name, item.value?.value]));
  ...
  while (state.snapshotOrder.length > 8) {
    ...
    if (expired) state.snapshots.delete(expired);
  }
...
async function action(params: Dynamic) {
  ...
  switch (params.kind) {
    ...
    case 'press_key':
      if (params.ref) await clickAt(await targetPoint(tabId, params), 'left', 1);
    ...
    case 'select':
      value = await callOnTarget(tabId, params.ref, function(this: HTMLSelectElement, option: string) {
        ...
        if (!item) throw new Error('select option was not found');
        this.value = item.value; this.dispatchEvent(new Event('input', { bubbles: true })); this.dispatchEvent(new Event('change', { bubbles: true }));
        return item.value;
      }, [params.option]);
      break;
    case 'check':
      value = await callOnTarget(tabId, params.ref, function(this: HTMLInputElement, checked: boolean) {
        if (!(this instanceof HTMLInputElement) || !['checkbox', 'radio'].includes(this.type)) throw new Error('target is not a checkbox or radio');
...
async function targetPoint(tabId: number, params: Dynamic): Promise<Point> {
  ...
  if (!quad || quad.length !== 8) throw new Error('target element has no visible box');
...
async function callOnTarget(tabId: number, ref: string, callback: Function, args: any[]) {
...
function pageState(tabId: number): PageState {
...
function invalidatePage(tabId: number) {
...
async function callPage(tabId: number, callback: Function, args: any[], returnByValue = true) {
  await attach(tabId);
...
async function dispatchKey(tabId: number, combination: string) {
  await attach(tabId);
  const parts = combination.split('+').map(value => value.trim()).filter(Boolean);
  const key = parts.pop();
  if (!key) throw new Error('key is required');
  const modifierValues: Record<string, number> = { alt: 1, control: 2, ctrl: 2, meta: 4, command: 4, shift: 8 };
...
function exceptionMessage(response: any) {
...
function numberLiteral(value: unknown, minimum: number, maximum: number) {
